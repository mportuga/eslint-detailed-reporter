...
function renderResults(results) {
	return _.map(results, function(result, index) {
		...
		// adds the source code to the results if it is not present
		// and there are problems in the file
		if (!result.source && !_.isEmpty(result.messages)) {
			result.source = fs.readFileSync(result.filePath, 'utf8');
		}

		template += renderSourceCode(result.source, result.messages, index);
